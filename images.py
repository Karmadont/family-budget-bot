"""
images.py — подготовка фотографии чека к отправке в Claude.
...
# Claude Opus 4.8 работает с картинками до 2576 px по длинной стороне.
# Всё, что больше, он уменьшит сам — платить за лишние пиксели смысла нет.
MAX_EDGE = 2576
...
def prepare(raw: bytes) -> tuple[bytes, str]:
    ...
    -> (байты, media_type) для блока image в запросе к Claude.
    """
    try:
