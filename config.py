...
def _required(name: str) -> str:
    ...
    Подсказку даём под оба случая. В контейнере файла .env нет и не должно быть —
    совет «заполните .env» там только сбивает с толку, а настоящее место
    настройки другое.
    """
    value = _env(name)
    if not value:
        _fail(
            ...
            "          На своей машине — впишите её в .env (см. .env.example).\n"
            "          На хостинге — задайте в разделе переменных и секретов "
...
# --- Устойчивость к обрывам связи с Telegram ---
#
# Российские хостинги ходят до api.telegram.org нестабильно: соединение не
# отвергается, а зависает, и запрос отваливается по таймауту. Само по себе это
# лечится повтором, но по умолчанию aiogram ждёт ответа 60 секунд — за это время
# человек успевает решить, что бот сломался, и написать сообщение заново.
#
# Поэтому у вызовов, от которых зависит, увидит ли человек ответ, свой короткий
# таймаут и свои повторы. Общий таймаут сессии не трогаем: по нему качаются фото
# чеков, а им лишние секунды нужны.
TELEGRAM_TIMEOUT = _float("TELEGRAM_TIMEOUT", "15")
TELEGRAM_RETRIES = _int("TELEGRAM_RETRIES", "3")
# Окно, внутри которого повторное сообщение с тем же текстом считается не второй
# покупкой, а попыткой человека добиться ответа. Ставьте 0, чтобы отключить.
DUPLICATE_WINDOW_MIN = _int("DUPLICATE_WINDOW_MIN", "10")

# --- Mini App (веб-интерфейс внутри Telegram) ---
# Публичный HTTPS-адрес, по которому открывается приложение. Пусто — веб-интерфейс
# выключен, бот работает как раньше. Telegram открывает только https и только с
# настоящим сертификатом: http и самоподписанные он молча не покажет.
WEBAPP_URL = _env("WEBAPP_URL").rstrip("/")
# Короткое имя Mini App из @BotFather (/newapp). Из него собирается ссылка
# t.me/<бот>/<короткое_имя> — единственный способ открыть приложение из группы.
WEBAPP_SHORT_NAME = _env("WEBAPP_SHORT_NAME", "budget")
# Где слушает встроенный веб-сервер. По умолчанию только localhost: наружу
# приложение выставляет Caddy или nginx, они же держат сертификат.
WEBAPP_HOST = _env("WEBAPP_HOST", "127.0.0.1")
WEBAPP_PORT = _int("WEBAPP_PORT", "8080")
# Сколько секунд подпись Telegram считается свежей. Сутки — как в примерах Telegram:
# приложение может долго висеть открытым, но вечной подпись быть не должна.
WEBAPP_AUTH_TTL = _int("WEBAPP_AUTH_TTL", "86400")
# Кому верить, когда он представляется чужим адресом в заголовке X-Forwarded-For.
# На своём сервере перед ботом стоит Caddy на этой же машине — значит 127.0.0.1.
# На хостинге вроде Amvera прокси живёт по неизвестному заранее адресу внутри их
# сети, и там ставят "*". От этого зависит только адрес клиента в логах: доступ к
# данным проверяется подписью Telegram и членством в чате, а не адресом.
WEBAPP_TRUSTED_PROXY = _env("WEBAPP_TRUSTED_PROXY", "127.0.0.1")
